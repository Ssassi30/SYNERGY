...
{
//chargement de l'image 
Masque->url="miniMAPSC_1.bmp";
Masque->img=SDL_LoadBMP(Masque->url);
if (Masque->img==NULL)
{
printf("unable to load background image %s \n",SDL_GetError());
return;
...
{
//chargement de l'image 
m->url="miniMAPSC_1.bmp";
m->img=SDL_LoadBMP(m->url);
if (m->img==NULL)
{
printf("unable to load background image %s \n",SDL_GetError());
return;
}
m->pos_img_ecran.x=600;
m->pos_img_ecran.y=0;
...
m->pos_img2_ecran.x=610;
m->pos_img2_ecran.y=180;
